...
if (!function_exists('calculateBalance')) {
    function calculateBalance($row, $normal_side = 'debit')
    {
        $debit = (float)($row['debit'] ?? 0);
        $credit = (float)($row['credit'] ?? 0);
        return ($normal_side === 'debit') ? ($debit - $credit) : ($credit - $debit);
    }
}
            ...
            <div class="page-header-content border-top carriers-page-header-content">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <form class="steps-basic clearfix" method="get" id="frm<?php echo $module; ?>" name="frm<?php echo $module; ?>" autocomplete="off" action="report_balance_sheet.php">
            ...
            <div class="table-responsive">
                <table class="table">
                    <tbody>
                        ...
                        <tr>
                            <th class="table-light" colspan="2">Assets</th>
                        </tr>
                        <?php
                        $total_assets = 0;
                        if (!empty($assets_accounts)) {
                            foreach ($assets_accounts as $row) {
                                $balance = calculateBalance($row, 'debit');
                        ...
                        <tr class="fw-semibold">
                            <td>Total Assets</td>
                            <td class="text-end"><?php echo BASE_CURRENCY['code'] . dec_($total_assets); ?></td>
                        </tr>

                        <tr>
                            <th class="table-light" colspan="2">Liabilities</th>
                        </tr>
                        <?php
                        $total_liabilities = 0;
                        if (!empty($liability_accounts)) {
                            foreach ($liability_accounts as $row) {
                                $balance = calculateBalance($row, 'credit');
                        ...
                        <tr class="fw-semibold">
                            ...
                            <td class="text-end"><?php echo BASE_CURRENCY['code'] . dec_($total_liabilities); ?></td>
                        </tr>

                        <tr>
                            <th class="table-light" colspan="2">Equity</th>
                        </tr>
                        <?php
                        $total_equity = 0;
                        if (!empty($equity_accounts)) {
                            foreach ($equity_accounts as $row) {
                                $balance = calculateBalance($row, 'credit');
                        ...
                        <?php } else { ?>
                            <tr>
                                <td colspan="2" class="text-muted">No equity accounts found.</td>
                            </tr>
                        <?php } ?>
                        <tr class="fw-semibold">
                            <td>Total Equity</td>
                            <td class="text-end"><?php echo BASE_CURRENCY['code'] . dec_($total_equity); ?></td>
                        </tr>

                        <tr class="fw-semibold table-light">
                            <td>Total Liabilities &amp; Equity</td>
                            <td class="text-end"><?php echo BASE_CURRENCY['code'] . dec_($total_liabilities + $total_equity); ?></td>
                        </tr>
