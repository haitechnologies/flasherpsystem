...
$module = 'expenses';
$module_caption = 'Expenses by Customer';
...
$targetpage            = 'report_expenses_by_customer.php?action=run_report&date_from=' . $date_from . '&date_to=' . $date_to . '&report_basis=' . $report_basis;
...
//COUNT QUERY - Get vendor count from expenses
$result         = $mysqli->query("SELECT COUNT(DISTINCT COALESCE(e.vendor_id, 'Internal')) as vendor_count FROM `{$TBL_EXPENSES}` e WHERE e.id>0 " . $expense_search_query);
$count_row = $result->fetch_array(MYSQLI_ASSOC);
$total_rows     = isset($count_row['vendor_count']) ? $count_row['vendor_count'] : 0;

//NORMAL QUERY - Get expenses grouped by vendor/customer with pagination
$result_customers  = $mysqli->query("SELECT 
                                COALESCE(v.display_name, 'Internal') as customer_name,
                                COUNT(e.id) as expense_count,
                                SUM(e.grand_total) as total_amount,
                                SUM(0) as total_tax
                            FROM `{$TBL_EXPENSES}` e 
                            LEFT JOIN `{$TBL_VENDORS}` v ON e.vendor_id = v.id
                            WHERE e.id > 0 " . $expense_search_query . "
                            GROUP BY COALESCE(v.display_name, 'Internal')
                            ORDER BY customer_name ASC LIMIT $start, $limit");

//EXPORT EXCEL - Get all expenses grouped by vendor/customer without limit
$result_customers_ = $mysqli->query("SELECT 
                                COALESCE(v.display_name, 'Internal') as customer_name,
                                COUNT(e.id) as expense_count,
                                SUM(e.grand_total) as total_amount,
                                SUM(0) as total_tax
...
$accounts_report_subcategory_id = getTableAttrv("id", tbl_accounts_report_subcategories, " slug = 'expenses_by_customer'");
            ...
            <div class="page-header-content">
                ...
                <div class="row mt-2">
                    <div class="col-lg-6">
                        ...
                        <div class="mb-0">
                            <span class="fw-semibold">Expenses by Customer</span> - <span class="small">From <?php echo ddm_($date_from); ?> To <?php echo ddm_($date_to); ?></span>
                        </div>
                    </div>
            ...
            <div class="page-header-content border-top carriers-page-header-content">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <form class="steps-basic clearfix" method="request" id="frm<?php echo $module; ?>" name="frm<?php echo $module; ?>" autocomplete="off" action="report_expenses_by_customer.php">
    ...
    <div class="content">
        ...
        <div class="card">
            <div class="card-header text-center">
                ...
                <h5 class="mb-0">Expenses by Customer</h5>
                ...
                <table class="table">
                    <tbody>
                        <tr>
                            ...
                            <th class="table-light text-end">EXPENSE COUNT</th>
                            <th class="table-light text-end">EXPENSE AMOUNT</th>
                            <th class="table-light text-end">EXPENSE AMOUNT WITH TAX</th>
                        </tr>
                        ...
                        // Use the pre-built result_customers query from pagination
                        while ($row_customers = $result_customers->fetch_array(MYSQLI_ASSOC)) {
                            $customer_name = !empty($row_customers['customer_name']) ? $row_customers['customer_name'] : 'Internal';
                            $expense_count = intval($row_customers['expense_count']);
                            $amount = floatval($row_customers['total_amount']);
                            $tax = floatval($row_customers['total_tax']);
                        ...
                        ?>
                            <tr>
                                ...
                                <td class="text-end"><?php echo $expense_count; ?></td>
                                <td class="text-end"><strong><?php echo BASE_CURRENCY['code']; ?> <?php echo number_format($amount, 2); ?></strong></td>
                                <td class="text-end"><strong><?php echo BASE_CURRENCY['code']; ?> <?php echo number_format($amount_with_tax, 2); ?></strong></td>
                            </tr>
                        <?php } //while 
                        ?>
                        ...
                        <tr style="background-color: #f5f5f5; font-weight: bold;">
                            <td>TOTAL:</td>
                            <td class="text-end"><?php echo $total_count; ?></td>
                            <td class="text-end"><?php echo BASE_CURRENCY['code']; ?> <?php echo number_format($total_amount, 2); ?></td>
                            <td class="text-end"><?php echo BASE_CURRENCY['code']; ?> <?php echo number_format($total_with_tax, 2); ?></td>
                        </tr>
                        <?php } ?>
