...
$module = 'customers';
$module_caption = 'Receivable Details';
...
$targetpage            = 'report_receivable_details.php?action=run_report&filter_by=' . $filter_by . '&date_from=' . $date_from . '&date_to=' . $date_to;
...
$accounts_report_subcategory_id = getTableAttrv("id", tbl_accounts_report_subcategories, " slug = 'receivable_details'");
            ...
            <div class="page-header-content border-top carriers-page-header-content">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <form class="steps-basic clearfix" method="get" id="frm<?php echo $module; ?>" name="frm<?php echo $module; ?>" autocomplete="off" action="report_receivable_details.php">
                ...
                <table class="table">
                    <tbody>
                        ...
                        if (!empty($date_from_ymd)) {
                            $date_filter .= " AND i.invoice_date >= '" . $date_from_ymd . "'";
                        }
                        ...
                        $sql_count = "
                            ...
                            INNER JOIN `" . tbl_invoices . "` i ON i.id = ii.invoice_id
                            INNER JOIN `" . tbl_customers . "` c ON c.id = i.customer_id
                            WHERE i.customer_id > 0 " . $date_filter . "
                        ";
                        ...
                        $sql_items = "
                            ...
                            INNER JOIN `" . tbl_invoices . "` i ON i.id = ii.invoice_id
                            INNER JOIN `" . tbl_customers . "` c ON c.id = i.customer_id
                            LEFT JOIN `" . tbl_items . "` itm ON itm.id = ii.service
                            WHERE i.customer_id > 0 " . $date_filter . "
                            ORDER BY c.display_name ASC, i.invoice_date DESC, ii.id ASC
                            LIMIT $start, $limit
                        ";
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($invoice_status == 'paid') { ?>
                                        <span class="badge bg-success"><?php echo ucfirst($invoice_status); ?></span>
                                    <?php } elseif ($invoice_status == 'unpaid' || $invoice_status == 'pending') { ?>
                                        <span class="badge bg-warning"><?php echo ucfirst($invoice_status); ?></span>
                                    <?php } elseif ($invoice_status == 'cancelled' || $invoice_status == 'void') { ?>
                                        <span class="badge bg-danger"><?php echo ucfirst($invoice_status); ?></span>
                                    <?php } else { ?>
                                        <span class="badge bg-secondary"><?php echo ucfirst($invoice_status); ?></span>
                                    <?php } ?>
                                </td>
                                <td>Invoice</td>
