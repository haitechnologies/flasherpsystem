...
$module = 'expenses';
$module_caption = 'Expense by Category';
...
$targetpage            = 'report_expenses_by_category.php?action=run_report&date_from=' . $date_from . '&date_to=' . $date_to . '&report_basis=' . $report_basis;
...
//COUNT QUERY - Get distinct categories count
$result         = $mysqli->query("SELECT COUNT(DISTINCT 'General') as category_count FROM `{$TBL_EXPENSES}` e WHERE e.id>0 " . $expense_search_query);
$count_row = $result->fetch_array(MYSQLI_ASSOC);
$total_rows     = isset($count_row['category_count']) ? $count_row['category_count'] : 0;

//NORMAL QUERY - Get expenses grouped by category with pagination
$result_categories  = $mysqli->query("SELECT 
                            ...
                            ORDER BY category_name ASC LIMIT $start, $limit");

//EXPORT EXCEL - Get all expenses grouped by category without limit
$result_categories_ = $mysqli->query("SELECT 
                                'General' as category_name,
                                SUM(e.grand_total) as total_amount,
                                SUM(0) as total_tax
                            FROM `{$TBL_EXPENSES}` e 
...
$accounts_report_subcategory_id = getTableAttrv("id", tbl_accounts_report_subcategories, " slug = 'expenses_by_category'");
            ...
            <div class="page-header-content">
                ...
                <div class="row mt-2">
                    <div class="col-lg-6">
                        ...
                        <div class="mb-0">
                            <span class="fw-semibold">Expense by Category</span> - <span class="small">From <?php echo ddm_($date_from); ?> To <?php echo ddm_($date_to); ?></span>
                        </div>
                    </div>
            ...
            <div class="page-header-content border-top carriers-page-header-content">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <form class="steps-basic clearfix" method="request" id="frm<?php echo $module; ?>" name="frm<?php echo $module; ?>" autocomplete="off" action="report_expenses_by_category.php">
        ...
        <div class="card">
            ...
            <div class="table-responsive">
                <table class="table">
                    <tbody>
                        <tr>
                            <th class="table-light">CATEGORY NAME</th>
                        ...
                        // Use the pre-built result_categories query from pagination
                        while ($row_categories = $result_categories->fetch_array(MYSQLI_ASSOC)) {
                            $category_name = !empty($row_categories['category_name']) ? $row_categories['category_name'] : 'Uncategorized';
                            $amount = floatval($row_categories['total_amount']);
                            $tax = floatval($row_categories['total_tax']);
