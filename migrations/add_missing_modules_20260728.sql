...
-- 1. Clean up any duplicate rows from a previous partial run
--    (ids differ between local and live, so use slug-based detection)
DELETE FROM erp_module_permissions WHERE module_id IN (
    SELECT id FROM erp_modules WHERE slug IN ('account_report_categories','account_report_subcategories') AND id NOT IN (100,101)
);
DELETE FROM erp_permissions WHERE module_id IN (
    SELECT id FROM erp_modules WHERE slug IN ('account_report_categories','account_report_subcategories') AND id NOT IN (100,101)
);
DELETE FROM erp_modules WHERE slug IN ('account_report_categories','account_report_subcategories') AND id NOT IN (100,101);

-- 2. FIX: Update existing module slugs from plural to singular
--    (code uses singular; existing rows have plural slugs)
UPDATE erp_modules SET slug = 'account_report_categories', module_name = 'Account Report Categories'
WHERE slug = 'accounts_report_categories' AND id = 100;

UPDATE erp_modules SET slug = 'account_report_subcategories', module_name = 'Account Report Subcategories'
WHERE slug = 'accounts_report_subcategories' AND id = 101;

-- 3. INSERT missing modules (air_tickets, gratuity_settlements)
...
-- 4. INSERT module permission types (view/create/edit/delete) for new modules
...
-- 5. Grant perms to System Admin (role_id=1) for new modules
...
-- 6. Grant perms to Super Admin (role_id=2) for new modules
INSERT IGNORE INTO erp_permissions (role_id, permission_id, module_id, publish, is_active, created_by, created_at)
...
-- 7. Also ensure account_report_categories/subcategories have all 4 perm types
INSERT IGNORE INTO erp_module_permissions (module_id, slug, permission_name, publish, is_active, created_by, created_at)
...
-- 8. Grant perms for account_report_categories/subcategories to role 1,2 if missing
INSERT IGNORE INTO erp_permissions (role_id, permission_id, module_id, publish, is_active, created_by, created_at)
SELECT 1, mp.id, mp.module_id, 1, 1, 1, NOW()
FROM erp_module_permissions mp
JOIN erp_modules m ON mp.module_id = m.id
